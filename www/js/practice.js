// practice.js
// 4ステップ（意味理解→多聴→シャドーイング→アウトプット）の進行を管理
//
// 録音・読み上げは実機ネイティブプラグインに依存するため、
// プラグイン未導入時（Web確認時）はモックで動作確認できるようにしている。
// 使用プラグイン:
//   - お手本読み上げ: @capacitor-community/text-to-speech
//   - シャドーイング音声録音(Step3) / アウトプット録音(Step4): capacitor-voice-recorder
//     + @capacitor/filesystem（ファイル保存）。どちらも採点はせず、後で聞き返せるように保存するのみ。
...
const PracticeController = {
  ...
  _goToSegment(index) {
    ...
    document.getElementById("step-indicator").textContent =
      `Step 1 / 4 ・ セグメント ${index + 1} / ${this.article.segments.length}`;
    ...
    // Step4: 録音ボタン/ステータスを新しいセグメント用にリセット（前セグメントの「✓ 録音済み」が残らないように）
    ...
    document.getElementById("btn-to-output").classList.add("hidden");
    document.getElementById("shadowing-recording-status").classList.add("hidden");

    // Step4: 英文表示（トグルがONの場合のみ、Glue Word/キーワードハイライト付きで表示）
  ...
  _showStep(stepName) {
    const steps = ["meaning", "listen", "shadow", "output"];
    steps.forEach((s) => {
  ...
  // Step3のボタンはStep4と同じトグル式（🎤 話してみる ⇄ ■ 終了）。採点はせず、
  // capacitor-voice-recorderで録音するだけで、後で聞き返せるように保存する。
  async recordShadowing() {
    ...
    if (!this.isRecordingShadowing) {
      ...
      try {
        ...
        console.error("シャドーイング音声の録音開始に失敗:", e);
        alert("録音を開始できませんでした。");
      }
      return;
    }
    ...
    if (this._shadowingAudioStarted) {
      try {
        ...
        console.warn("シャドーイング音声の保存に失敗:", e);
        this._showSaveWarning(statusEl);
      }
    }

    document.getElementById("btn-to-output").classList.remove("hidden");
  ...
  _showSaveWarning(statusEl) {
    if (!statusEl) return;
    statusEl.textContent = "今回は音声を保存できませんでした";
  ...
  // Step4の「セグメントの英文を表示する」トグル。設定は端末に記憶し、次回以降も維持する。
  ...
  // Step3(shadowing/)・Step4(recordings/)共通の録音ファイル保存処理。
  ...
  async finishSegment() {
    ...
    const existing = records[todayKey] ?? { segmentsDone: 0, totalSegments: this.article.segments.length };
    existing.segmentsDone += 1;
    existing.totalSegments = this.article.segments.length;

    const status = existing.segmentsDone >= existing.totalSegments ? "full" : "partial";
